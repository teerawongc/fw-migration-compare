/* ============================================================================
   FMC Time Range — Create & Bind  v1.2
...
// ── 2. คุมจังหวะไม่ให้เกินโควตา ────────────────────────────────────────────
// FMC จำกัด 120 request/นาที ต่อ token — การผูก rule ใช้ 2 request ต่อ 1 rule
// (GET ตัวเต็ม + PUT) ถ้าปล่อยยิงรัวๆ 738 rule จะชน 429 ตั้งแต่ครึ่งทาง แล้ว
// ติดหล่ม retry ยาว เพราะโควตาไม่ทันฟื้น จึงต้องคุมที่ต้นทางแทนการไปแก้ปลายทาง
// เก็บเวลาที่ยิงไปในหน้าต่าง 60 วินาทีล่าสุด ถ้าจะเกินเพดานก็รอจนกว่าคำขอที่เก่า
// ที่สุดจะหลุดออกจากหน้าต่าง
const RATE = 100;          // ต่ำกว่าเพดานจริง 120 ไว้เผื่อ request จากหน้าเว็บ FMC ที่เปิดค้างอยู่
let hits = [];
async function throttle() {
  for (;;) {
    const now = Date.now();
    hits = hits.filter(t => now - t < 60000);
    if (hits.length < RATE) { hits.push(now); return; }
    const wait = 60000 - (now - hits[0]) + 50;
    if (wait > 1500) log('  ถึงโควตา ' + RATE + ' req/นาที — พัก ' + Math.ceil(wait / 1000) + ' วิ');
    await sleep(wait);
  }
}

// ── ตัวเรียก API พร้อม retry ───────────────────────────────────────────────
...
async function api(method, path, body, tries = 8) {
  for (let t = 1; t <= tries; t++) {
    ...
    if (r.status === 429) {
      // โดนแล้วแปลว่าโควตาหมดจริง ต้องรอให้หน้าต่างฟื้น ไม่ใช่ถอยทีละ 3 วิ
      const wait = Math.min(60000, 10000 * t);
      warn('  โดน rate limit รอ ' + Math.round(wait / 1000) + ' วิ (ครั้งที่ ' + t + '/' + tries + ')');
      hits = [];                       // ล้างตัวนับ เริ่มจับจังหวะใหม่หลังพัก
      await sleep(wait);
      continue;
    }
...
// ห่อทั้งลูปไว้ เพื่อให้ยังได้รายงานออกมาแม้จะพังกลางทาง — งานนี้ใช้เวลาเป็น
// สิบนาที ถ้าพังแล้วไม่รู้ว่าทำถึงไหนจะเสียเวลามาไล่ดูใน FMC ทีละ rule
try {
for (const [rid, item] of plan) {
  ...
  // เช็คจาก list ที่โหลดมาแล้ว (expanded=true มี timeRangeObjects ติดมาด้วย)
  // ก่อนตัดสินใจ GET — ทำให้รันซ้ำหลังพังกลางทางแทบไม่กิน request เลย
  const listed = (item.rule.timeRangeObjects || []).map(o => o.name);
  if (want.every(n => listed.includes(n))) {
    skipN++;
    report.push([item.rule.name, rid, want.join('|'), 'SKIP', 'ผูกไว้อยู่แล้ว']); continue;
  }

  if (DRY) {
    okN++; log(label + ' — จะผูก: ' + want.join(', '));
    report.push([item.rule.name, rid, want.join('|'), 'DRY', 'จะผูก ' + want.join('|')]); continue;
  }

  // ต้อง GET ตัวเต็มก่อน PUT เสมอ — ตัวที่ได้จาก list ยังไม่ครบทุก field
  ...
  // FMC คืน rule ตัวเต็มหลัง PUT มาให้อยู่แล้ว ใช้ตัวนั้นตรวจได้เลย ไม่ต้อง GET ซ้ำ
  // — ประหยัดไป 1 ใน 3 ของ request ทั้งงาน ซึ่งคือส่วนที่ทำให้ชน 429 ตั้งแต่แรก
  const after = fp(p.body);
  if (after !== before) {
    failN++;
  ...
  const got = (p.body.timeRangeObjects || []).map(o => o.name);
  ...
  bad('หยุดกลางทางที่ rule ที่ ' + idx + '/' + plan.size + ': ' + e.message);
  warn('รันสคริปต์ใหม่ได้เลย — rule ที่ผูกไปแล้วจะขึ้น SKIP แล้วทำต่อจากจุดที่ค้าง');
}
